...
> **Home/Profile refinement amendment — 2026-07-14 (additive):** A founder-directed refinement of this spec's Home presentation is authorized and implemented: central Twinko at 132–145 pt (compact ≥118 pt) with separate inner (creamy) and outer (pink-lavender) glow layers and a 4 s float / ~4.8 s breathing idle loop (Reduce Motion keeps a static glow); the 2–2–1 orbit tightened into one central cluster (Chat/Tarot ≈34–44 pt and Zodiac/Meditate ≈28–38 pt from Twinko's edge); §10's Profile control is now a **temporary founder-approved native-SwiftUI planet icon** (no PNG asset exists; no initial/letters); and §11's Profile menu opens a **branded bottom sheet** (misty lavender surface, custom rounded rows — not a system grouped List) with a header (planet, name, zodiac summary, Edit) and an Edit Profile flow persisting locally. Settings contains Language only; Log Out remains excluded. Everything else in this spec remains active.
>
